...
import { Message } from "@/db/interfaceDB";

const ChatBottomBar = () => {
	const [message, setMessage] = useState("");
  const [uploadMediaType,setuploadMediaType] = useState<"text"| "image"| "video"| "files">('image')
  ...
  const uploadMedia = (result: CloudinaryUploadWidgetResults, widget: any) => {
   
    const uploadInfo = result.info as CloudinaryUploadWidgetInfo;
    setImgUrl((result.info as CloudinaryUploadWidgetInfo).secure_url);
    switch(uploadInfo.resource_type){
      case 'image': setuploadMediaType("image")
                    break;
      case 'video': setuploadMediaType('video')
                    break;
      case 'raw': setuploadMediaType('files')
                    break;
		...
		<div className='p-2 flex justify-between w-full items-center gap-2'>
			{!message.trim() && (
				<CldUploadWidget
					...
					onSuccess={(result, { widget }) => uploadMedia(result,widget)}
				>
					{({ open }) => {
						return (
							<ImageIcon
			...
			<Dialog open={!!imgUrl}>
              ...
              : (
                <ReactPlayer width='' controls url={imgUrl} />
              )
              }
					</div>
					...
					<DialogFooter>
						<Button
							type='submit'
							onClick={() => {
								sendMessage({ content: imgUrl, messageType: uploadMediaType, receiverId: selectedUser?.id! });
								setImgUrl("");
							}}
						>
							Send
